...
```{r activityHistogram, message=FALSE, warning=FALSE}
library (ggplot2) 
...
```
Mean is **`r stepsmean`** and median **`r stepsmedian`** is steps taken per day.
...
```
The time interval with the maximum average number of steps has code **`r maxIntervalCode`** which means time interval **`r maxInterval`**.
...
```{r imputedHistogram, warning=FALSE}
...
```
After replacement of NA values by interval's mean, mean became equal to **`r imputedmean`** and median to **`r imputedmedian`** steps taken per day. The median differs from one of the first part of the study, but mean remains the same. Now median is equal to mean. 
